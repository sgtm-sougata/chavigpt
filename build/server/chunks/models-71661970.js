import { H as HF_TOKEN, O as OPENAI_API_KEY, b as MODELS, d as OLD_MODELS, e as HF_API_ROOT } from './database-97662a5e.js';
...
async function endpointOai(input) {
  ...
  let OpenAI;
  try {
    OpenAI = (await import('./index-8c2ab54f.js').then(function (n) { return n.a; })).OpenAI;
  } catch (e) {
...
${previousUserMessages.map(({ content }) => `- ${content}`).join("\n")}` : "";
      ...
      return openAIChatToTextGenerationStream(
        await openai.chat.completions.create({
          model: model.id ?? model.name,
          messages: conversation.preprompt ? [{ role: "system", content: conversation.preprompt }, ...messagesOpenAI] : messagesOpenAI,
          stream: true,
          max_tokens: model.parameters?.max_new_tokens,
          stop: model.parameters?.stop,
          temperature: model.parameters?.temperature,
...
//# sourceMappingURL=models-71661970.js.map
