...
![Chat UI repository thumbnail](https://huggingface.co/datasets/huggingface/documentation-images/resolve/main/chavigpt-websearch.png)
...
Read the full tutorial [here](https://huggingface.co/docs/hub/spaces-sdks-docker-chavigpt#chavigpt-on-spaces).
...
```bash
docker run -d -p 27017:27017 --name mongo-chavigpt mongo:latest
```
...
```env
PUBLIC_APP_NAME=chavigpt
PUBLIC_APP_ASSETS=chavigpt
...
- `PUBLIC_APP_ASSETS` Is used to find logos & favicons in `static/$PUBLIC_APP_ASSETS`, current options are `chavigpt` and `huggingchat`.
...
If `endpoints` are left unspecified, chavigpt will look for the model on the hosted Hugging Face inference API using the model name.
